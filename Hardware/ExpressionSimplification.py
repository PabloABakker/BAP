...
def distill_algorithm(kcm_df, expression_strings=None, expression_boundaries=None):
    ...
    # Generate seeds to select best columns
    seeds = generate_distill_seeds(binary_matrix)
    ...
    for i, seed in enumerate(seeds):
        expanded_rect = expand_rectangle_greedily(seed, binary_matrix, kcm_df, cube_columns, term_id_matrix, expression_strings, expression_boundaries)
...
def greedy_kernel_intersection_algorithm(expression_strings):
    ...
    while True:
        ...
        best_rect = distill_algorithm(kcm_matrix, analysable_expressions, expression_boundaries)
        
        if best_rect is None:
            print("No best rectangle found")
            break
        
        print("Best rectangle found:")
        print("Rows:", best_rect['rows'])
        print("Columns:", best_rect['columns'])
        print("Cokernels:", best_rect['cokernels'])
        print("Cost:", best_rect['cost'])
        
 
        
        # Create a PolynomialTerm function from the rectangle
        function_name = "G" + str(iteration)
        ...
        # Need to replace the original expression with its factored form expressed in terms of the new function
  
        current_expressions = update_expressions_with_extracted_function(current_expressions, analysable_expressions, extracted_function, best_rect, expression_boundaries)
            
        print("Updated expressions: ", current_expressions)
