...
class CustomDynamicsEnv(gym.Env):
    ...
    def step(self, action):
        ...
        err = np.abs(sol.y[:, -1]) - np.abs(self.state)
        ...
        # Calculate reward
        reward = -np.sum(1*np.square(self.state[2]) + 0.1*np.square(self.state[3])) + err[2] + err[3] # penalize u, w, theta
...
register(
    id="testEnv-v2",
    entry_point="teeeest:CustomDynamicsEnv",
